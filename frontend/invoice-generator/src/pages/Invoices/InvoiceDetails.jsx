...
const InvoiceDetails = () => {
  ...
  const handlUpdate = async (formData) => {
    try{
  ...
  if(isEditing){
    return <CreateInvoice existingInvoice={invoice} onSave={handlUpdate} />
  }
  ...
  if(!invoice){
    return(
      <div className="flex flex-col items-center justify-center py-12 text-center bg-slate-50 rounded-lg ">
        ...
        <h3 className="txet-lg font-medium text-slate-900 mb-2">Invoice Not Found</h3>
  ...
  return (
    <>
      <ReminderModal isOpen={isReminderModelOpen} onClose={() => setIsReminderModelOpen(false)} invoiceId={id} />
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 print:hidden">
        ...
        <div className="flex items-center gap-2">
          {invoice.status !== 'Paid' && (
            <Button variant="secondary" onClick={() => setIsReminderModelOpen(true)} icon={Mail}>General Reminder</Button>
          )}
        ...
        <div ref={invoiceRef} id="invoice-preview" className="bg-white p-6 sm:p-8 md:p-12 rounded-lg shadow-md border-slate-200 pb[100vh]">
          <div className="flex flex-col sm:flex-row justify-between items-start pb-8 border-b border-slate-200">
            ...
            <div className="text-left sm:text-right mt-4 sm:mt-0 ">
              <p className="txet-sm text-slate-500 mt-2">Status</p>
